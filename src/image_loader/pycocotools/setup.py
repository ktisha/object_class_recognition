...
for split in ['train', 'val']:
    for anno in ['instances', 'sentences']:
        # download annotations
        if split == 'train' and anno == 'instances':
            size = '324'
        elif split == 'val' and anno == 'instances':
            size = '157'
        elif split == 'train' and anno == 'sentences':
            size = '61'
        elif split == 'val' and anno == 'sentences':
            size = '30'
        ...
        if query_yes_no("Do you want to download %s split for %s annotations [%sMB] under ./annotations?"%(split, anno, size), default='yes'):
            ...
            url = 'http://msvocds.blob.core.windows.net/annotations-0-9/%s_%s2014.json'%(anno, split)
            download(url, fname)